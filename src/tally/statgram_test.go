...
func TestParseSample(t *testing.T) {
    ...
    if expected != sample { t.Errorf("expected %#v, got %#v", expected, sample) }
...
func TestParseStatgramLine(t *testing.T) {
    ...
    if err != nil { t.Error("expected empty statgram, got error:", err) }
    if len(statgram) > 0 {
    ...
    if err != nil { t.Error("expected statgram, got error:", err) }
